...
const Income = () => {
  const { totalIncome, householdId } = useExpenses();
      ...
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 px-1">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-xl md:text-3xl font-black text-white tracking-tighter uppercase">Nodos de Entrada</h1>
            </div>
            <p className="text-[8px] md:text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] ml-13">Monitoreo de flujo entrante</p>
          </div>
          <button 
          ...
          >
            {!isInitializing && <div className="absolute inset-0 bg-emerald-500 -translate-x-full group-hover:translate-x-0 transition-transform duration-500" />}
            <span className="relative z-10 group-hover:text-white flex items-center gap-2 transition-colors">
              ...
              {isInitializing ? 'Inicializando hogar...' : 'Inyectar Capital'}
            </span>
          </button>
        </header>
          ...
          <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-5 md:p-6 bg-white/[0.01] border border-white/5 rounded-[24px] md:rounded-3xl flex items-center gap-4">
               <div className="w-10 h-10 md:w-12 md:h-12 shrink-0 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-center">
                  <Banknote className="w-4 h-4 md:w-5 md:h-5 text-slate-600" />
               </div>
               <div>
                  <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest">Mayor Inyección</p>
            ...
            <div className="p-5 md:p-6 bg-white/[0.01] border border-white/5 rounded-[24px] md:rounded-3xl flex items-center gap-4">
               ...
               <div>
                  <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest">Próxima Fecha</p>
                  <p className="text-base md:text-lg font-bold text-white leading-tight">---</p>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-950/40 backdrop-blur-xl p-5 md:p-8 rounded-[32px] md:rounded-[40px] border border-white/5 shadow-2xl relative">
          <div className="flex items-center gap-3 mb-6 md:mb-8">
            ...
            <h3 className="text-[10px] md:text-xs font-black uppercase tracking-[0.4em] text-white">Log de Entradas Recientes</h3>
          </div>
