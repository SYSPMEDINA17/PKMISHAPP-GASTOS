...
export const ExpenseChart = () => {
  ...
  return (
    <motion.div 
      ...
      className="bg-slate-950/40 backdrop-blur-xl p-5 md:p-8 rounded-[24px] md:rounded-[32px] border border-white/5 shadow-2xl min-h-[380px] md:min-h-[440px] flex flex-col relative overflow-hidden group"
    >
      {/* Luz de fondo sutil */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 blur-[80px]" />
      
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 md:gap-6 mb-8 md:mb-10 relative z-10">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Activity className="w-4 h-4 text-cyan-500 animate-pulse" />
            <h3 className="text-[10px] md:text-sm font-black uppercase tracking-[0.3em] text-white">Monitoreo de Flujos</h3>
          </div>
          <p className="text-[8px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest pl-6">
            Ciclo Operativo: {monthName} {year}
          </p>
        </div>
        
        <div className="flex items-center gap-4 md:gap-6 p-2 bg-white/[0.02] border border-white/5 rounded-2xl px-4">
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]" />
            <span className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest">Ingresos</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-rose-400 shadow-[0_0_8px_rgba(244,63,94,0.5)]" />
            <span className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest">Gastos</span>
          </div>
        </div>
      </div>

      <div className="w-full relative mt-2 flex-1">
        {loading ? (
      ...
      <div className="mt-6 md:mt-8 flex justify-between items-center text-[8px] md:text-[9px] font-bold text-slate-700 uppercase tracking-[0.3em]">
        <div className="flex items-center gap-2">
